				...
				<section class="footer-panel p-6 sm:p-7">
					<a class="group mb-5 inline-flex items-center gap-3" href="#home" aria-label="مدارس الندى - الصفحة الرئيسية">
						...
						<div class="leading-tight">
							<div class="font-heading text-lg font-bold text-white">
								{{ \App\Models\SiteSetting::get("footer_brand_line1", "مدارس الندى") }}</div>
							<div class="text-xs text-primary-300">
								{{ \App\Models\SiteSetting::get("footer_brand_line2", "النموذجية الأهلية") }}</div>
						</div>
					</a>
					...
					<dl class="mt-6 grid grid-cols-2 gap-3">
						...
						<div class="footer-stat-card">
							...
							<dd class="footer-stat-value">{{ \App\Models\SiteSetting::get("footer_stat2_value", "اعتماد وزارة التعليم") }}
							</dd>
						</div>
						<div class="footer-stat-card sm:col-span-2">
							...
							<dd class="footer-stat-value">
								{{ \App\Models\SiteSetting::get("footer_stat3_value", "بيئة تعليمية واضحة ومساندة للأسرة من أول تواصل") }}</dd>
						</div>
					</dl>
					...
					<div class="mb-5 flex flex-wrap items-start justify-between gap-3">
						<div>
							...
							<h3 class="font-heading text-lg font-bold text-white">
								{{ \App\Models\SiteSetting::get("footer_nav_title", "كل ما يحتاجه ولي الأمر في مكان واحد") }}</h3>
		...
		<div class="mt-8 border-t border-white/10 py-4">
			<div class="flex flex-col items-center justify-between gap-4 text-xs text-primary-400 sm:flex-row">
				<p>
					...
					{{ $year }}
					{{ \App\Models\SiteSetting::get("footer_copyright", "مدارس الندى النموذجية الأهلية. جميع الحقوق محفوظة.") }}
				</p>

				<nav class="flex flex-wrap items-center justify-center gap-4 sm:me-8" aria-label="روابط التذييل السفلي">
