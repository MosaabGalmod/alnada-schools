<div x-data="{ tab: 'contact' }" lang="ar" dir="rtl">
	...
	@if ($flashMsg)
		<div
			class="{{ $flashType === "error"
			    ...
			    : "bg-primary-50 text-primary-700 border border-primary-200 dark:bg-primary-900/20 dark:text-primary-400 dark:border-primary-800" }} mb-6 flex items-center gap-3 rounded-2xl px-5 py-3.5 text-sm font-medium"
	...
	<div class="mb-6 overflow-x-auto">
		<div
			class="flex min-w-max gap-1 rounded-2xl border border-gray-100 bg-gray-50/80 p-1 dark:border-gray-700 dark:bg-gray-800/60"
			role="tablist" aria-label="أقسام الإعدادات">
			@foreach ([["contact", "التواصل", "M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"], ["branding", "الهوية", "M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"], ["seo", "السيو", "M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"], ["footer", "التذييل", "M4 6h16M4 12h16M4 18h7"], ["strings", "النصوص", "M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"]] as [$key, $label, $icon])
				<button
	...
	{{-- ══════════════════════════════════════════════════
     |  TAB: التواصل
     ══════════════════════════════════════════════════ --}}
	<div role="tabpanel" aria-label="معلومات التواصل" x-show="tab === 'contact'" x-transition>
		<form class="space-y-6" wire:submit="saveContact">
			{{-- Contact Info --}}
			<div
				class="overflow-hidden rounded-3xl border border-gray-50 bg-white shadow-card dark:border-gray-700 dark:bg-gray-800">
				<div
					class="flex items-center gap-3 border-b border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-gray-700 dark:bg-gray-700/50">
					<div class="flex h-9 w-9 items-center justify-center rounded-xl bg-primary-100 dark:bg-primary-900/30">
						<svg class="h-5 w-5 text-primary-700 dark:text-primary-400" fill="none" stroke="currentColor"
							viewBox="0 0 24 24">
				...
				<div class="grid gap-5 p-6 md:grid-cols-2">
					@foreach ([["instagram", "إنستغرام", "@"], ["twitter", "تويتر/X", "@"], ["facebook", "فيسبوك", "fb/"], ["whatsapp", "واتساب (رقم)", "+"]] as [$field, $lbl, $prefix])
						<div>
							...
							<div class="relative">
								<span
									class="pointer-events-none absolute inset-y-0 end-3.5 flex items-center text-xs text-gray-400 dark:text-gray-500"
									dir="ltr">{{ $prefix }}</span>
								<input class="form-input pe-8" id="{{ $field }}" type="text" wire:model="{{ $field }}"
									dir="ltr">
							</div>
							@error($field)
								<p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
	...
	{{-- ══════════════════════════════════════════════════
     |  TAB: الهوية
     ══════════════════════════════════════════════════ --}}
	<div role="tabpanel" aria-label="هوية الموقع" x-show="tab === 'branding'" x-cloak x-transition>
		<div class="space-y-6">
			{{-- Site Name & Tagline --}}
			<div
				class="overflow-hidden rounded-3xl border border-gray-50 bg-white shadow-card dark:border-gray-700 dark:bg-gray-800">
				<div
					...
					<div class="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-100 dark:bg-indigo-900/30">
						<svg class="h-5 w-5 text-indigo-700 dark:text-indigo-400" fill="none" stroke="currentColor"
							viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
			...
			<div
				...
				<div
					...
					<div class="flex h-9 w-9 items-center justify-center rounded-xl bg-purple-100 dark:bg-purple-900/30">
						<svg class="h-5 w-5 text-purple-700 dark:text-purple-400" fill="none" stroke="currentColor"
							viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
				...
				<div class="p-6">
					<div class="flex flex-wrap items-start gap-5">
						<div class="flex flex-col items-center gap-2">
							<div
								class="flex h-20 w-20 items-center justify-center overflow-hidden rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-700">
								@if ($currentLogo)
									<img class="h-auto max-h-16 w-auto max-w-[4rem] object-contain"
										src="{{ \Illuminate\Support\Facades\Storage::disk("public")->url($currentLogo) }}" alt="الشعار الحالي">
								@else
									<img class="h-auto max-h-14 w-auto max-w-[3.5rem] object-contain opacity-60" src="{{ asset("logo.svg") }}"
						...
						<div class="min-w-[200px] flex-1 space-y-3">
							<div class="flex flex-wrap gap-2">
								<label
									class="btn-sm cursor-pointer bg-primary-50 text-primary-700 hover:bg-primary-100 dark:bg-primary-900/30 dark:text-primary-300"
									for="logo-upload">
								...
								<input class="sr-only" id="logo-upload" type="file" wire:model="logoFile"
									accept="image/png,image/jpeg,image/svg+xml,image/webp">
								@if ($logoFile)
									<button class="btn-sm bg-green-50 text-green-700 hover:bg-green-100 dark:bg-green-900/30 dark:text-green-300"
										type="button" wire:click="uploadLogo">
										<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								...
								@endif
								@if ($currentLogo)
									<button class="btn-sm bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-900/30 dark:text-red-400"
										type="button" wire:click="deleteLogo" wire:confirm="حذف الشعار والعودة للافتراضي؟">
										<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
							...
							@if ($logoFile)
								<div class="flex items-center gap-2">
									<div
										class="h-10 w-10 overflow-hidden rounded-xl border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-700">
										<img class="h-full w-full object-contain" src="{{ $logoFile->temporaryUrl() }}" alt="">
									</div>
									<span class="text-xs text-gray-500 dark:text-gray-400">{{ $logoFile->getClientOriginalName() }}</span>
								</div>
							@endif
							<div class="flex items-center gap-1.5 text-xs text-primary-600" wire:loading wire:target="logoFile">
			...
			<div
				...
				<div
					...
					<div class="flex h-9 w-9 items-center justify-center rounded-xl bg-teal-100 dark:bg-teal-900/30">
						<svg class="h-5 w-5 text-teal-700 dark:text-teal-400" fill="none" stroke="currentColor"
							viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
				...
				<div class="p-6">
					<div class="flex flex-wrap items-start gap-5">
						<div class="flex flex-col items-center gap-2">
							<div
								class="flex h-16 w-16 items-center justify-center overflow-hidden rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-700">
								@if ($currentFavicon)
									<img class="h-8 w-8 object-contain"
										src="{{ \Illuminate\Support\Facades\Storage::disk("public")->url($currentFavicon) }}"
										alt="الأيقونة الحالية">
								@else
									<img class="h-8 w-8 object-contain opacity-60" src="{{ asset("logo-sm.png") }}" alt="الأيقونة الافتراضية">
								@endif
							</div>
						...
						<div class="min-w-[200px] flex-1 space-y-3">
							<div class="flex flex-wrap gap-2">
								<label
									class="btn-sm cursor-pointer bg-teal-50 text-teal-700 hover:bg-teal-100 dark:bg-teal-900/30 dark:text-teal-300"
									for="favicon-upload">
									<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
											d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
								...
								<input class="sr-only" id="favicon-upload" type="file" wire:model="faviconFile"
									accept="image/png,image/jpeg,image/x-icon,image/webp">
								@if ($faviconFile)
									<button class="btn-sm bg-green-50 text-green-700 hover:bg-green-100 dark:bg-green-900/30 dark:text-green-300"
										type="button" wire:click="uploadFavicon">
										<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
										</svg>
										رفع الأيقونة
	...
	{{-- ══════════════════════════════════════════════════
     |  TAB: السيو
     ══════════════════════════════════════════════════ --}}
	<div role="tabpanel" aria-label="إعدادات السيو" x-show="tab === 'seo'" x-cloak x-transition>
			...
			<div
				...
				<div
					...
					<div class="flex h-9 w-9 items-center justify-center rounded-xl bg-green-100 dark:bg-green-900/30">
						<svg class="h-5 w-5 text-green-700 dark:text-green-400" fill="none" stroke="currentColor"
							viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
				...
				<div class="space-y-5 p-6">
					<div>
						...
						@error("seo_title_default")
							<p class="mt-1 text-xs text-red-500">{{ $message }}</p>
						@enderror
					</div>
					<div>
						<label class="form-label" for="seo_description_default">الوصف الافتراضي</label>
						<textarea class="form-input min-h-[80px] resize-y" id="seo_description_default" wire:model="seo_description_default"
						 rows="3" placeholder="مدارس الندى النموذجية الأهلية – مؤسسة تعليمية رائدة في التربية الخاصة والدمج..."></textarea>
						<p class="mt-1.5 text-xs text-gray-400">ما يظهر تحت الرابط في جوجل (≤ 160 حرف مثالي)</p>
						@error("seo_description_default")
							<p class="mt-1 text-xs text-red-500">{{ $message }}</p>
						@enderror
					</div>
					<div>
	...
	{{-- ══════════════════════════════════════════════════
     |  TAB: التذييل
     ══════════════════════════════════════════════════ --}}
	<div role="tabpanel" aria-label="محتوى التذييل" x-show="tab === 'footer'" x-cloak x-transition>
			...
			<div
				...
				<div
					...
					<div class="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-100 dark:bg-blue-900/30">
						<svg class="h-5 w-5 text-blue-700 dark:text-blue-400" fill="none" stroke="currentColor"
							viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
				...
				<div class="grid gap-5 p-6 md:grid-cols-2">
					<div class="md:col-span-2">
						...
						<input class="form-input" id="footer_cta_badge" type="text" wire:model="footer_cta_badge"
							placeholder="قبول وتسجيل وبرامج متخصصة تحت إشراف تعليمي معتمد">
						@error("footer_cta_badge")
							<p class="mt-1 text-xs text-red-500">{{ $message }}</p>
						@enderror
					</div>
					<div>
						<label class="form-label" for="footer_cta_title">العنوان</label>
						<input class="form-input" id="footer_cta_title" type="text" wire:model="footer_cta_title"
							placeholder="لنبدأ الرحلة التعليمية المناسبة لابنكم">
					...
					<div class="md:col-span-2">
						...
						<textarea class="form-input min-h-[72px] resize-y" id="footer_cta_desc" wire:model="footer_cta_desc" rows="2"
						 placeholder="فريقنا يجيب عن الاستفسارات، يشرح البرامج، ويرتب زيارة تعريفية..."></textarea>
						@error("footer_cta_desc")
							<p class="mt-1 text-xs text-red-500">{{ $message }}</p>
						@enderror
					</div>
				</div>
			...
			<div
				...
				<div class="grid gap-5 p-6 md:grid-cols-2">
					<div>
						...
						<input class="form-input" id="footer_brand_line1" type="text" wire:model="footer_brand_line1"
							placeholder="مدارس الندى">
					</div>
					<div>
						<label class="form-label" for="footer_brand_line2">السطر الثاني</label>
						<input class="form-input" id="footer_brand_line2" type="text" wire:model="footer_brand_line2"
							placeholder="النموذجية الأهلية">
					</div>
					<div class="md:col-span-2">
						<label class="form-label" for="footer_about_desc">وصف المدرسة</label>
						<textarea class="form-input min-h-[72px] resize-y" id="footer_about_desc" wire:model="footer_about_desc"
						 rows="2" placeholder="رائدة في التربية الخاصة والدمج بالمدينة المنورة منذ عام 1429هـ..."></textarea>
					</div>
					{{-- Stat cards --}}
					@foreach ([["footer_stat1_label", "footer_stat1_value", "بداية التميز", "1429هـ"], ["footer_stat2_label", "footer_stat2_value", "المرجعية", "اعتماد وزارة التعليم"], ["footer_stat3_label", "footer_stat3_value", "التجربة", "بيئة تعليمية واضحة ومساندة للأسرة"]] as [$lField, $vField, $lPlaceholder, $vPlaceholder])
						<div>
							<label class="form-label">بطاقة إحصاء — التسمية</label>
							<input class="form-input" type="text" wire:model="{{ $lField }}"
								placeholder="{{ $lPlaceholder }}">
						</div>
						<div>
							<label class="form-label">بطاقة إحصاء — القيمة</label>
							<input class="form-input" type="text" wire:model="{{ $vField }}"
				...
				<div
					...
					<div class="space-y-4 p-5">
						<div>
							...
							<input class="form-input" id="footer_nav_kicker" type="text" wire:model="footer_nav_kicker"
								placeholder="خريطة سريعة">
						</div>
						<div>
							<label class="form-label text-xs" for="footer_nav_title">العنوان</label>
							<input class="form-input" id="footer_nav_title" type="text" wire:model="footer_nav_title"
								placeholder="كل ما يحتاجه ولي الأمر في مكان واحد">
						</div>
						<div>
							<label class="form-label text-xs" for="footer_nav_desc">الوصف</label>
							<textarea class="form-input min-h-[60px] resize-y" id="footer_nav_desc" wire:model="footer_nav_desc" rows="2"
							 placeholder="روابط أساسية مختصرة لتقليل التمرير..."></textarea>
						</div>
						<div>
							<label class="form-label text-xs" for="footer_nav_chip">الشريحة</label>
							<input class="form-input" id="footer_nav_chip" type="text" wire:model="footer_nav_chip"
								placeholder="تنقل مباشر وواضح">
						</div>
					</div>
				</div>
				...
				<div
					...
					<div class="space-y-4 p-5">
						<div>
							...
							<input class="form-input" id="footer_contact_kicker" type="text" wire:model="footer_contact_kicker"
								placeholder="قنوات مباشرة">
						</div>
						<div>
							<label class="form-label text-xs" for="footer_contact_title">العنوان</label>
							<input class="form-input" id="footer_contact_title" type="text" wire:model="footer_contact_title"
								placeholder="تواصل سريع دون بحث إضافي">
						</div>
						<div>
							<label class="form-label text-xs" for="footer_contact_desc">الوصف</label>
							<textarea class="form-input min-h-[60px] resize-y" id="footer_contact_desc" wire:model="footer_contact_desc"
							 rows="2" placeholder="كل وسائل التواصل الأساسية وساعات العمل..."></textarea>
						</div>
						<div>
							<label class="form-label text-xs" for="footer_copyright">نص حقوق النشر</label>
							<input class="form-input" id="footer_copyright" type="text" wire:model="footer_copyright"
								placeholder="مدارس الندى النموذجية الأهلية. جميع الحقوق محفوظة.">
						</div>
					</div>
				</div>
			</div>
	...
	{{-- ══════════════════════════════════════════════════
     |  TAB: النصوص
     ══════════════════════════════════════════════════ --}}
	<div role="tabpanel" aria-label="نصوص الواجهة" x-show="tab === 'strings'" x-cloak x-transition>
			...
			<div
				...
				<div
					...
					<div class="flex h-9 w-9 items-center justify-center rounded-xl bg-orange-100 dark:bg-orange-900/30">
						<svg class="h-5 w-5 text-orange-700 dark:text-orange-400" fill="none" stroke="currentColor"
							viewBox="0 0 24 24">
				...
				<div class="grid gap-5 p-6 md:grid-cols-3">
					<div>
						...
						<input class="form-input" id="nav_brand_line1" type="text" wire:model="nav_brand_line1"
							placeholder="مدارس الندى">
					</div>
					<div>
						<label class="form-label" for="nav_brand_line2">السطر الثاني</label>
						<input class="form-input" id="nav_brand_line2" type="text" wire:model="nav_brand_line2"
							placeholder="النموذجية الأهلية">
					</div>
					<div>
			...
			<div
				...
				<div class="grid gap-5 p-6 md:grid-cols-2">
					<div>
						...
						<input class="form-input" id="about_year_label" type="text" wire:model="about_year_label"
							placeholder="هـ - سنة التأسيس">
					</div>
					<div>
						<label class="form-label" for="about_year_sub">النص الفرعي</label>
						<input class="form-input" id="about_year_sub" type="text" wire:model="about_year_sub"
							placeholder="أكثر من 15 عامًا من التميز والعطاء...">
					</div>
					<div>
						<label class="form-label" for="about_badge1_title">شارة الاعتماد — العنوان</label>
						<input class="form-input" id="about_badge1_title" type="text" wire:model="about_badge1_title"
							placeholder="جودة معتمدة">
					</div>
					<div>
						<label class="form-label" for="about_badge1_sub">شارة الاعتماد — النص الفرعي</label>
						<input class="form-input" id="about_badge1_sub" type="text" wire:model="about_badge1_sub"
							placeholder="وزارة التعليم">
					</div>
					<div>
						<label class="form-label" for="about_badge2_title">شارة البيئة — العنوان</label>
						<input class="form-input" id="about_badge2_title" type="text" wire:model="about_badge2_title"
							placeholder="بيئة آمنة">
					</div>
					<div>
						<label class="form-label" for="about_badge2_sub">شارة البيئة — النص الفرعي</label>
						<input class="form-input" id="about_badge2_sub" type="text" wire:model="about_badge2_sub"
							placeholder="ومحفّزة للإبداع">
					</div>
					<div class="md:col-span-2">
						<label class="form-label" for="about_cta_text">نص زر التواصل</label>
						<input class="form-input" id="about_cta_text" type="text" wire:model="about_cta_text"
			...
			<div class="grid gap-6 lg:grid-cols-3">
				<div
					...
					<div class="p-5">
						...
						<textarea class="form-input min-h-[80px] resize-y" id="testimonials_intro" wire:model="testimonials_intro"
						 rows="3" placeholder="تجارب حقيقية من أولياء أمور وثقوا في مدارس الندى — بأصواتهم وبكلماتهم."></textarea>
						@error("testimonials_intro")
							<p class="mt-1 text-xs text-red-500">{{ $message }}</p>
						@enderror
					</div>
				</div>
				<div
					...
					<div class="p-5">
						...
						<textarea class="form-input min-h-[80px] resize-y" id="why_us_intro" wire:model="why_us_intro" rows="3"
						 placeholder="نرتب تجربة ولي الأمر حول محاور واضحة..."></textarea>
						@error("why_us_intro")
							<p class="mt-1 text-xs text-red-500">{{ $message }}</p>
						@enderror
					</div>
				...
				<div
					...
					<div class="space-y-3 p-5">
						<div>
							...
							<input class="form-input" id="contact_kicker" type="text" wire:model="contact_kicker"
								placeholder="جاهزون للاستماع">
						</div>
						<div>
							<label class="form-label text-xs" for="contact_intro_title">العنوان</label>
							<input class="form-input" id="contact_intro_title" type="text" wire:model="contact_intro_title"
								placeholder="قنوات واضحة للتواصل...">
						</div>
						<div>
							<label class="form-label text-xs" for="contact_intro_text">النص</label>
							<textarea class="form-input min-h-[60px] resize-y" id="contact_intro_text" wire:model="contact_intro_text"
							 rows="2" placeholder="اختر الوسيلة الأسرع لك..."></textarea>
						</div>
					</div>
				</div>
				...
				<div class="settings-card">
					...
					<div class="grid gap-4 sm:grid-cols-2">
						<div>
							...
							<input class="form-input" id="contact_form_kicker" type="text" wire:model="contact_form_kicker"
								placeholder="راسلنا مباشرة">
						</div>
						<div>
							<label class="form-label text-xs" for="contact_form_title">العنوان</label>
							<input class="form-input" id="contact_form_title" type="text" wire:model="contact_form_title"
								placeholder="أرسل لنا رسالة">
						</div>
						<div class="sm:col-span-2">
							<label class="form-label text-xs" for="contact_form_text">النص التوضيحي</label>
							<textarea class="form-input min-h-[60px] resize-y" id="contact_form_text" wire:model="contact_form_text"
							 rows="2" placeholder="املأ النموذج وسيتابع فريق المدرسة..."></textarea>
						</div>
					</div>
				</div>
			</div>
