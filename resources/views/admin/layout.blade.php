			...
			<aside class="admin-sidebar relative z-30 flex shrink-0 flex-col overflow-hidden transition-all duration-300"
						...
						<a class="admin-nav-item {{ request()->routeIs('admin.' . $key) ? 'active' : '' }}"
							...
							<span class="relative shrink-0">
								...
								@if ($hasBadge && ($unreadCount ?? 0) > 0)
									<span class="absolute -right-1.5 -top-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[9px] font-bold leading-none text-white">
							...
							@if ($hasBadge && ($unreadCount ?? 0) > 0)
								<span class="ml-auto min-w-[20px] rounded-full bg-red-500 px-1.5 py-0.5 text-center text-xs font-bold leading-none text-white"
				...
				<div class="admin-sidebar-footer space-y-0.5 p-2">
					<a class="admin-nav-item" href="{{ route('home') }}" title="عرض الموقع" target="_blank" rel="noopener noreferrer">
						<svg class="h-5 w-5 shrink-0 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
			...
			<div class="flex min-w-0 flex-1 flex-col overflow-hidden">
				...
				<header class="admin-topbar sticky top-0 z-20 flex items-center justify-between gap-3 px-4 py-3 lg:px-6">
					...
					<div class="flex shrink-0 items-center gap-2">
						...
						<div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-primary-600 to-primary-800 text-sm font-bold text-white shadow-clay">
							م
						</div>
					</div>
				</header>
