...
<section class="section custom-section" id="{{ $section->key }}" data-dark="{{ $isDark ? "1" : "0" }}"
	data-testid="custom-section" aria-labelledby="{{ $headingId }}"
	style="{{ $section->bgCss() ?: "" }} {{ $sectionVars }}" lang="ar" dir="rtl">
							...
							<article class="custom-card custom-card-bullets" data-testid="custom-bullets">
								...
								<ul class="custom-bullet-list" role="list">
									@foreach ($block["items"] as $item)
										<li>
											<svg class="custom-bullet-check" aria-hidden="true" viewBox="0 0 20 20" fill="currentColor">
