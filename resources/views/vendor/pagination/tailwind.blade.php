@if ($paginator->hasPages())
	<nav class="flex flex-wrap items-center justify-between gap-3 pt-2" role="navigation" aria-label="التنقل بين الصفحات"
		dir="rtl">
		...
		<div class="flex items-center gap-1">
			...
			@else
				<a class="admin-page-btn" href="{{ $paginator->previousPageUrl() }}" aria-label="الصفحة السابقة" rel="prev">
					<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
					</svg>
				</a>
			...
			@foreach ($elements as $element)
				...
				@if (is_array($element))
					@foreach ($element as $page => $url)
						...
						@else
							<a class="admin-page-btn" href="{{ $url }}"
								aria-label="الصفحة {{ $page }}">{{ $page }}</a>
						@endif
					@endforeach
				@endif
			@endforeach
			...
			@if ($paginator->hasMorePages())
				<a class="admin-page-btn" href="{{ $paginator->nextPageUrl() }}" aria-label="الصفحة التالية" rel="next">
					<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
